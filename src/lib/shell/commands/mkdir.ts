...
function mkdir({ command: { positional }, dir }: AccessObject) {
    ...
    if (!dir.read(parentDir)) return `mkdir: cannot create directory ‘${requestedDir}’: Not a directory`;
    // Check if dir or file already exists
    if (dir.read(requestedDir)) return `mkdir: cannot create directory ‘${requestedDir}’: File exists`;
    // Create directory
